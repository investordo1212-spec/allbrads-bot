...
def aplicar_pago(sheet, fila, row, monto):
    ...
    for i, mes in enumerate(MESES):
        ...
        if valor_actual == "" or valor_actual == "⬜":
            deuda_mes = cuota
        else:
            try:
...
def webhook():
    ...
    if num_media > 0 and estado["paso"] == "inicio":
        ...
        msg.body("📋 Comprobante recibido.\n\nResponde con el nombre y monto en este formato:\n\n_Apellido, monto_\n\nEjemplo:\n_Herreros, 20000_")

    # PASO 2: recibe nombre y monto
    elif estado["paso"] == "esperando_datos":
        try:
            ...
            if len(resultados) == 0:
                msg.body(f"❌ No encontré a *{nombre_raw}* en el registro.\n\nVerifica el apellido e intenta de nuevo.")
                conversaciones[telefono] = {"paso": "esperando_datos"}
    ...
    # PASO 4: confirmación final
        ...
        if mensaje.upper() in ["SI", "SÍ", "S", "YES", "Y"]:
            try:
                sheet = get_sheet()
                resumen = aplicar_pago(sheet, estado["fila"], estado["row"], estado["monto"])
                resumen_texto = "\n".join(resumen)
        ...
        conversaciones.pop(telefono, None)
        msg.body("👋 Para registrar un pago, envía una *foto del comprobante*.")
